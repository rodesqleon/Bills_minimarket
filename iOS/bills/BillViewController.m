...
{
    self = [super initWithNibName:@"BillView" bundle:nibBundleOrNil];
...
- (void)add{
    ...
    if(self.initial_bill <= self.end_bill){
        ...
        [self showMessage:@"Please, check bill number because is over the limit"
                withTitle:@"Error"];
    }
    [[self navigationItem] setRightBarButtonItems:[NSArray                                          new]];
    UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithTitle:@"Terminar" style:UIBarButtonItemStylePlain
...
- (void)delete{
    if(self.initial_bill <= self.initial_bill_static){
        [self showMessage:@"No hay más elementos que elimnar"
                withTitle:@"Error"];
    }else{
        NSMutableDictionary *detail = [NSMutableDictionary new];
...
- (IBAction)finish:(UIButton *)sender {
    ...
    if([self.bill count] == 0){
        ...
        dispatch_async(downloadQueue, ^{
            ...
            NSString *var7 = [var6 stringByAppendingString:[NSString stringWithFormat:@"%d",total]];
            NSString *var8 = [var7 stringByAppendingString:@"&initial_bill="];
            NSString *var9 = [var8 stringByAppendingString:[NSString stringWithFormat:@"%d",self.initial_bill_static]];
            NSString *var10 = [var9 stringByAppendingString:@"&end_bill="];
            NSString *post = [var10 stringByAppendingString:[NSString stringWithFormat:@"%d",self.end_bill]];
            ...
            [request setURL:[NSURL URLWithString:@"http://always420.cl/wsAddBill.php"]];
